...
::: notes
Monthly Hgb measurements and EPO doses But the data is unbalanced.
:::
...
(\beta_6Age_i + \beta_7Sex_i + \beta_7Dose_{ij} + \beta_8Iron_{ij})t_{ij} + \\
(\beta_9Age_i + \beta_{10}Sex_i + \beta_{11}Iron_{ij})Dose_{ij} + \\
...
-   Comprare models with Gaussian serial correlation, exponential serial correlation, and without serial correlation using REML log-likelihood
:::
...
::: {.column width="30%"}
-   Model assuming a linear relationship between each patient's Hgb and time: $R^2_{meta} = 0.52$
-   Model assuming a quadratic relationship: $R^2_{meta} = 0.73$
-   F-test for addition of quadratic to linear relationship: p \<0.0001
:::
:::

::: notes
The patients have a different Hb at the start of the experiment and this also changes with time. We modeled individual Hb over time and R squared and F_test suggested that a quadratic relationship with time may explain the data better than linear relationship
:::
...
::: notes
Change of Hgb level from baseline to the last measured level varied dramatically. Majority of patients tended to have minor change in Hgb levels. Increase in age leads to increase in AUC. Disadvantages: only use partial information so not insightful
:::
...
- The mean Hgb concentration on the first 1 is 10.92 g/dl (ranging from 4.45 to 18.77)
- The mean slope is 0.03 (ranging from -5 to 7.1)

::: notes
A linear regression model between Hgb and time was fitted for each patient to explain within-subject variability. 
The mean Hgb concentration on the first 1 is 10.92 g/dl (ranging from 4.45 to 18.77) and the mean slope is 0.03 (ranging from -5 to 7.1)

Note: this slide can be removed for the interest of time
:::
...
|--------------|-------------|--------|------------------------|--------------------|-------------|-------------|
| Intercept    | Intercept   | 2931   | 10.6090               | 0.1077             | 98.53       | <0.0001     |
| AGE          | Slope       | 1      | 0.00499               | 0.00168            | 2.97        | 0.0030      |

- A unit increase in age is associated with a slight increase (0.005 g/dl) in the baseline Hgb level.

::: notes
In this analysis we examined the effect of age on the trend of Hgb levels across months. The goal was to model the between-subject variability. 
:::
...
| **Variable** | **Label**   | **DF** | **Parameter Estimate** | **Standard Error** | **T Value** | **P-value** |
|--------------|-------------|--------|------------------------|--------------------|-------------|-------------|
| Intercept    | Intercept   | 2931   | 0.1568                | 0.05532            | 2.83        | 0.0046      |
| AGE          | Slope       | 1      | -0.00187              | 0.000862           | -2.17       | 0.0300      |
...
|------------------------------|--------------------------------------|-------------|--------------------------------------|-------------|
| **Intercept**                | 10.6218 (0.0885)                    | <0.0001     | 10.6123 (0.0884)                    | <0.0001     |
| **EPO dose**                 | 0.0005 (0.0002)                     | 0.0046      | 0.0005 (0.0002)                     | 0.0059      |
| **Age**                      | 0.0027 (0.0013)                     | 0.0421      | 0.0028 (0.0013)                     | 0.0329      |
| **Male**                     | 0.1008 (0.0393)                     | 0.0102      | 0.1037 (0.0392)                     | 0.0083      |
| **Iron deficiency**          | 0.1662 (0.0317)                     | <0.0001     | 0.1682 (0.0318)                     | <0.0001     |
| **Month**                    | -0.0282 (0.0156)                    | 0.0706      | -0.0375 (0.0155)                    | 0.0154      |
| **EPO dose * month**         | 0.0005 (0.0001)                     | <0.0001     | 0.0006 (0.0001)                     | <0.0001     |
| **Iron deficiency * month**  | -0.0433 (0.0182)                    | 0.0174      | -0.0471 (0.0184)                    | 0.0106      |
...
- Over all, Hgb decreases over time, though it's not significant in LMM, but only in multivariate model.
- EPO increases Hgb over time
- Iron-deficient patients decreases Hgb over time
